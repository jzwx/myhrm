...
<tr bgcolor="#EEEEEE">
 ...
 <%
	...
	<tr align="center" bgcolor="#FFFFFF">
		...
		<a href="searchworkers.jsp?nowPage=1">[首页]</a>
		<%
		if(pg.getNowPage()>1){
		 %>
		<a href="searchworkers.jsp?nowPage=<%=pg.getNowPage()-1 %>">[上一页]</a>
		<%
		}else{
		%>
		<a href="searchworkers.jsp?nowPage=1">[上一页]</a>
		<%
		}
		 %>
		<c:forEach var="i" begin="1" end="<%=pg.getpageCount("hr_workers") %>" step="1">
			<a href="searchworkers.jsp?nowPage=${i}">[${i}]</a>
		</c:forEach>
		<%
		if(pg.getNowPage()<pg.getpageCount("hr_workers")){
		%>
		<a href="searchworkers.jsp?nowPage=<%=pg.getNowPage()+1 %>">[下一页]</a>
		<%
		}else{
		%>
		<a href="searchworkers.jsp?nowPage=<%=pg.getpageCount("hr_workers")%>">[下一页]</a>
		<%
		}
		 %>
		<a href="searchworkers.jsp?nowPage=<%=pg.getpageCount("hr_workers")%>">[尾页]</a><span style="float:right;">总共<%=pg.getpageCount("hr_workers") %>页/当前是第<%=pg.getNowPage() %>页</span>
		</td>
	</tr>
<%
}else{
